...
try:
    ...
    logging.error(
        """See http://www.imgseek.net/isk-daemon/documents-1/compiling""")
    traceback.print_exc()
    sys.exit()
...
SUPPORTED_IMG_EXTS = [
    ...
    'rast',
    'xbm',
    'bmp']  # to help determining img format from extension
...
def normalizeResults(results):
    ...
    for i in range(len(results) / 2):
        ...
        rsc = results[i * 2 + 1]
        # normalize #TODO is this normalization factor still valid?
        rsc = -100.0 * rsc / 38.70
        # sanity checks
        if rsc < 0:
            rsc = 0.0
...
class ImgDB:
    ...
    def loadalldbs(self, fname):
        try:
            ...
            log.debug(
                '| Database (%s) loaded with %d spaces' %
                (fname, dbCount))
    ...
    def addDir(self, dbId, path, recurse):
        ...
        for fil in os.listdir(path):
            fil = safe_str(fil)
            fil = path + os.sep + fil
            if len(fil) > 4 and fil.split(
                    '.')[-1].lower() in SUPPORTED_IMG_EXTS:
                try:
    ...
    def addImageBlob(self, dbId, data, newid=None):
        ...
        if res != 0:  # add successful
            ...
            if self._settings.core.getboolean('database', 'automaticSave') and time.time(
            ) - dbSpace.lastSaveTime > self._settings.core.getint('database', 'saveInterval'):
                dbSpace.lastSaveTime = time.time()
                self.savealldbs()
        return res
    ...
    def queryImgIDFastKeywords(
            ...
            kwJoinType,
            keywords):
        return imgdb.queryImgIDFastKeywords(
            dbId,
            imgId,
            numres,
            kwJoinType,
    ...
    def queryImgIDKeywords(
        ...
        # do query
        results = imgdb.queryImgIDKeywords(
            dbId,
            imgId,
            numres,
            kwJoinType,
