...
core = ConfigParser.SafeConfigParser({
    ...
    'startAsDaemon': 'false',
    # base tcp port to start listening at for HTTP requests (admin interface,
    # XML-RPC requests, etc)
    'basePort': '31128',
    # print debug messages to console
    'debug': 'true',
    # automatically save all database spaces on server shutdown
    'saveAllOnShutdown': 'true',
    ...
    'automaticSave': 'false',
    # run in cluster mode ? If True, make sure subsequent settings are ok
    'isClustered': 'false',
    'seedPeers': 'isk2host:31128',
    # hostname for this instance. Other instances may try to connect to this
    # hostname
...
if len(conffile) < 1:
    logger.warn(
        '| no config file (isk-daemon.conf) found. Looked at local dir, home user dir and /etc/iskdaemon. Using defaults for everything.')
else:
