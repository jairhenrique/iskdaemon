    ...
    try:
        ...
        for pv in split(pathvar, ':'):
            if os.path.exists(pv +
                              '/Magick++-config') or os.path.exists(pv +
                                                                    'Magick++-config'):
                fnd = 1
        if fnd:
            IMagCFlag = os.popen(
                "Magick++-config --cxxflags --cppflags").read()
...
# force C++ linking
from distutils import sysconfig
# TODO-2 is this really still necessary?
...
setup(
    ...
    platforms=[
        ...
        'Mac OSX'],
    cmdclass={
        'build_ext': fallible_build_ext},
    ext_modules=[
        Extension(
            "_imgdb",
            [
                "imgSeekLib/imgdb.cpp",
                "imgSeekLib/haar.cpp",
                "imgSeekLib/imgdb.i",
